...
@Registers.datasets.register_with_name('custom_aligned_with_context')
class CustomAlignedDataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        super().__init__()
        self.image_size = (dataset_config.image_size, dataset_config.image_size)
        image_paths_ori = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, f'{stage}/B'))
        image_paths_cond = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, f'{stage}/A'))
        image_paths_context = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, f'{stage}/C'))
        
        self.flip = dataset_config.flip if stage == 'train' else False
        self.to_normal = dataset_config.to_normal

        # self.imgs_ori = ImagePathDatasetWithContext(image_paths_ori, self.image_size, context = True, flip=self.flip, to_normal=self.to_normal)
        # self.imgs_cond = ImagePathDatasetWithContext(image_paths_cond, self.image_size, context = False, flip=self.flip, to_normal=self.to_normal)
        
        self.imgs_ori = ImagePathDataset(image_paths_ori, self.image_size, flip=self.flip, to_normal=self.to_normal)
        self.imgs_cond = ImagePathDataset(image_paths_cond, self.image_size, flip=self.flip, to_normal=self.to_normal)
        self.imgs_context = ImagePathDataset(image_paths_context, self.image_size, flip=self.flip, to_normal=self.to_normal)

    def __len__(self):
        return len(self.imgs_ori)

    def __getitem__(self, i):
        # print(self.imgs_ori[i][1].shape)
        # print(self.imgs_context[i])
        return self.imgs_ori[i], self.imgs_cond[i], self.imgs_context[i][0]
