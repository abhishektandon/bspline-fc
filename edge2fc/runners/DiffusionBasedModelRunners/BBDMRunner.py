...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def get_latent_mean_std(self):
        ...
        def calc_mean(batch, total_ori_mean=None, total_cond_mean=None):
            if self.config.model.conditional == True:
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        ...
        if self.config.model.conditional == True:
            loss, additional_info = net(x, x_cond, context)
        else:
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        if self.config.model.conditional == True:
            image_grid = get_image_grid(context.to('cpu'), grid_size, to_normal=self.config.data.dataset_config.to_normal)
            im = Image.fromarray(image_grid)
